...
struct JudgeConnectionsSettingsView: View {
    ...
    private var saved: EvaluationJudgeConnection? {
        store.judgeConnections.first { $0.id == draft.id }
    }
    ...
    var body: some View {
        HSplitView {
            List(selection: $selectedID) {
                ...
                if saved == nil {
                    Label("New connection", systemImage: "plus.circle").tag(draft.id)
                }
            }
            .frame(minWidth: 190, idealWidth: 210)
            .safeAreaInset(edge: .bottom) {
                HStack {
                    Button("Add connection", systemImage: "plus") { startNew() }
                        .labelStyle(.iconOnly)
            ...
            VStack(spacing: 0) {
                Form {
                    ...
                    if let message {
                        ...
                        Text("Save your changes before checking this connection.")
                            .font(.caption).foregroundStyle(.secondary)
                    }
                }
                .formStyle(.grouped)
        ...
        .alert("Delete this connection?", isPresented: $isDeleting) {
            ...
            Button("Delete", role: .destructive) {
                do {
                    try store.deleteJudgeConnection(id: draft.id)
                    startNew()
                } catch { message = error.localizedDescription }
            }
        } message: {
            Text("The connection and its saved API key will be removed from this Mac.")
        }
    }
