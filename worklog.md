...
# Shared expanded time axis for sequential short spans

- User showed that enlarging bars individually made distinct submillisecond steps appear simultaneous. Replaced independent minimum widths with one monotonic time mapping shared by every bar and axis label. Closely spaced event boundaries receive visible separation; longer intervals share remaining width proportionally. All trace boundaries participate, so collapsing rows does not move the axis. Shared endpoints and true overlaps remain aligned; timestamps/duration text are unchanged.
- No zoom or horizontal scrolling was restored. Details stay permanent. The timeline explicitly identifies its expanded scale; legacy traces without offsets retain unavailable timing.
- Validation: 14 headless tests passed with swift test --scratch-path /tmp/foundation-ui-regressions --filter 'WorkflowTimelineIntervalTests|TimelineRenderingTests'. Coverage includes sequential rendered bars, parent/child overlap, shared endpoints, inverse axis labels, final spans, invalid zero-length timings, and 2,000 densely clustered events. Xcode BuildProject(buildForTesting: true) and git diff --check passed. On-screen automation remained off because of the previously reported display flashing.

# Toolbar progress capsule spacing and UI regressions

- User supplied a crop showing the progress track and counter touching the capsule's curved ends. Extracted the existing progress content into RunToolbarProgress and added 12-point horizontal insets. Counts stay on one line and the content keeps its intrinsic width, so longer counts grow the toolbar item rather than compressing the 90-point track. The native toolbar still supplies the bubble.
