...
The paired-host runs saved `developerExecution` provenance correctly. Their
AI-rubric sample status remained `unscored`: the feature-adapter runner applies
deterministic `MetricScorer` modes directly, while `.modelJudge` intentionally
maps to `unscored` until a later reassessment. This is current behavior, not a
claim that the remote feature passed the rubric.
...
3. The run-destination help says the same suite is run on the device, while an
   AI-rubric suite is collected as unscored and needs explicit reassessment.
   The product should either perform the configured judge step or make this
   collection-only boundary explicit before the run.
4. On the same Mac, clicking Disconnect left the current Multipeer browser with
   `The runner is no longer discoverable.` until the isolated apps relaunched.
   Saved-trust reconnection then succeeded without a new code. Physical-device
   verification is needed before treating this as a product defect rather than
   a same-host Multipeer limitation.
...
- `swift test` — passed: 44 tests in seven portable suites and 13 tests in
